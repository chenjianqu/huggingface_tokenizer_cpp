...
bool _is_chinese_char(UChar32 c) {
  ...
  if ((c >= 0x4E00 && c <= 0x9FFF) ||   // CJK Unified Ideographs
      ...
      (c >= 0x2F800 &&
       c <= 0x2FA1F)) { // CJK Compatibility Ideographs Supplement
    return true;
  }
  return false;
}
...
wstring pad_chinese_chars(const wstring &text) {
  ...
  for (auto &c : text) {
    if (_is_chinese_char(static_cast<UChar32>(c))) {
      vec_padded_chars.push_back(
          L' '); // wide-character representation of space
...
vector<wstring>
run_split_on_punctuation(const wstring &text, bool split_specials,
                         const vector<wstring> &special_tokens) {
  if (!split_specials && find(special_tokens.begin(), special_tokens.end(),
                              text) != special_tokens.end()) {
    // we do not want to split special tokens and we found the text in the
    // vector of special tokens
    ...
    while (start < input.length()) {
      ...
      if (best_match_length > 0) {
        ...
        result.push_back(input.substr(
            start, (found_next ? next_pos - start : std::wstring::npos)));
        start = next_pos;
      }
    }
...
int WordPieceTokenizer::get_word_index(const wstring &word) {
...
vector<size_t> WordPieceTokenizer::tokenize_full(const wstring &input_text,
                                                 bool split_specials) {
  ...
  for (size_t i = 0; i < special_word_tokenized.size(); i++) {
    ...
    basic_tokenized.insert(basic_tokenized.end(), splitted_by_punc.begin(),
...
vector<wstring>
WordPieceTokenizer::wordpiece_tokenize(const wstring &input_text) {
